...
#Train the logistic model for combined
logisticCombinedModel <- glm(Label.x ~ TD.y + Yds.y + Yds.G + Fum + FumL + Avg.y + TD.x + QBR + Pct + Yds.x + YdsG + Int, data=totalData)
summary(logisticCombinedModel)
...
#Obtain predictions
predictionsMain <- predict(logisticCombinedModel, mainData,type="response")
